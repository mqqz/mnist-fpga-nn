module uart_rx #(
  ...
  localparam [15:0] HALF_CYCLE_COUNT = HALF_CYCLE[15:0];
  //state machine code
  localparam S_IDLE = 1;
  ...
  always @(*) begin
    case (state)
      ...
      S_START:
      if (cycle_cnt == CYCLE_COUNT - 1'b1)  //one data cycle
        next_state = S_REC_BYTE;
      else next_state = S_START;
      S_REC_BYTE:
      if (cycle_cnt == CYCLE_COUNT - 1'b1 && bit_cnt == 3'd7)  //receive 8bit data
        next_state = S_STOP;
      else next_state = S_REC_BYTE;
      S_STOP:
      if (cycle_cnt == HALF_CYCLE_COUNT - 1'b1)  //half bit cycle,to avoid missing the next byte receiver
        next_state = S_DATA;
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    else if ((state == S_REC_BYTE && cycle_cnt == CYCLE_COUNT - 1'b1) || next_state != state)
      cycle_cnt <= 16'd0;
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    else if (state == S_REC_BYTE && cycle_cnt == HALF_CYCLE_COUNT - 1'b1) rx_bits[bit_cnt] <= rx_pin;
